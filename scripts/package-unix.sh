...
javac \
  ...
  -d "$BUILD_DIR/classes" \
  $(find src/v2 -name '*.java' | sort)

cp -R src/v2/resources "$BUILD_DIR/classes/src/v2/resources"
...
if [[ "$(uname -s)" == "Linux" ]]; then
  ICON_ARGS+=(--icon src/v2/resources/madlibs.png)
elif [[ "$(uname -s)" == "Darwin" ]]; then
  ICONSET="$BUILD_DIR/MadLibs.iconset"
  ICNS="$BUILD_DIR/MadLibs.icns"
  rm -rf "$ICONSET"
  mkdir -p "$ICONSET"
  sips -z 16 16 src/v2/resources/madlibs.png --out "$ICONSET/icon_16x16.png" >/dev/null
  sips -z 32 32 src/v2/resources/madlibs.png --out "$ICONSET/icon_16x16@2x.png" >/dev/null
  sips -z 32 32 src/v2/resources/madlibs.png --out "$ICONSET/icon_32x32.png" >/dev/null
  sips -z 64 64 src/v2/resources/madlibs.png --out "$ICONSET/icon_32x32@2x.png" >/dev/null
  sips -z 128 128 src/v2/resources/madlibs.png --out "$ICONSET/icon_128x128.png" >/dev/null
  sips -z 256 256 src/v2/resources/madlibs.png --out "$ICONSET/icon_128x128@2x.png" >/dev/null
  sips -z 256 256 src/v2/resources/madlibs.png --out "$ICONSET/icon_256x256.png" >/dev/null
  sips -z 512 512 src/v2/resources/madlibs.png --out "$ICONSET/icon_256x256@2x.png" >/dev/null
  sips -z 512 512 src/v2/resources/madlibs.png --out "$ICONSET/icon_512x512.png" >/dev/null
  sips -z 1024 1024 src/v2/resources/madlibs.png --out "$ICONSET/icon_512x512@2x.png" >/dev/null
...
JPACKAGE_ARGS=(
  ...
  --add-modules java.desktop,java.net.http,javafx.controls
  "${JPACKAGE_PREVIEW_ARGS[@]}"
  "${ICON_ARGS[@]}"
)

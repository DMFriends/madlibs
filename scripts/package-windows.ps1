...
$Sources = Get-ChildItem -Recurse -Filter *.java (Join-Path $ProjectDir "src\v2") | ForEach-Object { $_.FullName }
& $Javac --module-path $FxSdkLib --add-modules javafx.controls -d $Classes $Sources
if ($LASTEXITCODE -ne 0) { throw "javac failed" }

Copy-Item (Join-Path $ProjectDir "src\v2\resources") (Join-Path $Classes "src\v2") -Recurse
...
& $JPackage `
    ...
    --win-menu `
    --win-shortcut `
    --icon (Join-Path $ProjectDir "src\v2\resources\madlibs.ico") `
    --dest $DistDir
